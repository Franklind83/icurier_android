...
import com.dev.todos.Adapter.OrderSummaryMessageAdapter;
...
public class OrderSummaryMessageFragment extends Fragment {

    RecyclerView recyclerView;
    private KProgressHUD hud;
    String orderId, bringProduct, delDate, usetName, userProfile, userId2,userId1, userstatus;
    boolean isFormMyOrder = false;
    Button btnCancel, btnBring, btnSendMessage;
    TextView txtOrderContain, txtTotal;
    ArrayList<SingleOrderListResponce.OrderOfferList> order_offer_list;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        return inflater.inflate(R.layout.fragment_order_summary_message, container, false);
    }
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        view.findViewById(R.id.btnSendMessage).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {

                Bundle bundle = new Bundle();
                bundle.putString(Keys.user2_id,userId2);
                bundle.putString(Keys.user1_id,userId1);
        ...
        orderListResponceCall.enqueue(new Callback<SingleOrderListResponce>() {
            ...
            public void onResponse(Call<SingleOrderListResponce> call, Response<SingleOrderListResponce> response) {
                ...
                if (response.body().getStatus().equals(Keys.status_succes)) {
                    ...
                    Total =  Double.parseDouble(response.body().getOrder_details().getTotal_order_price())+ Double.parseDouble(response.body().getOrder_details().getTotal_delivery_reward());
                    ...
                    if (!isFormMyOrder) {
                        if(response.body().getOrder_details().getProduct_list().size() == 1) {
                            txtOrderContain.setText(getString(R.string.ordercontain) + response.body().getOrder_details().getProduct_list().size() + getString(R.string.product));
                        }else{
                            txtOrderContain.setText(getString(R.string.ordercontain) + response.body().getOrder_details().getProduct_list().size() + getString(R.string.product)+"s");
                    ...
                    recyclerView.setAdapter(new OrderSummaryMessageAdapter(getActivity(), getFragmentManager(),
                            response.body().getOrder_details().getProduct_list(), isFormMyOrder, orderId,response.body().getOrder_details()));
    ...
    public void showDialog(final String orderId) {
        ...
        a.setPositiveButton(getActivity().getString(R.string.ok), new DialogInterface.OnClickListener() {
            @Override
